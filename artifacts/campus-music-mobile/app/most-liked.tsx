...
import { useGetMostLikedTracks } from "@workspace/api-client-react";
...
export default function MostLikedScreen() {
  ...
  const { data: mostLikedRaw, isLoading: isLoadingMostLiked } = useGetMostLikedTracks(
    { limit: 100 },
    { query: { queryKey: ["most-liked", 100], staleTime: 0 } }
  );

  const allMostLiked: Track[] = (mostLikedRaw ?? []).map((t) => ({
    ...t,
  ...
  const filteredTracks = selectedGenre === "All"
    ? allMostLiked
    : allMostLiked.filter((t) => t.genre === selectedGenre);
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={[styles.header, { paddingTop: topPad + 8 }]}>
        ...
        <Text style={[styles.title, { color: colors.foreground }]}>Most Liked</Text>
        <View style={{ width: 24 }} />
      </View>
      ...
      {isLoadingMostLiked ? (
        ...
        <View style={styles.empty}>
          ...
          <Text style={[styles.emptyText, { color: colors.mutedForeground }]}>
            {selectedGenre === "All" ? "No liked tracks yet" : `No liked ${selectedGenre} tracks yet`}
          </Text>
        </View>
      ) : (
        <FlatList
            ...
            return (
              ...
              >
                ...
                <View style={styles.likesBadge}>
                  ...
                  <Text style={[styles.likesText, { color: colors.primary }]}>
...
const styles = StyleSheet.create({
  ...
  likesBadge: { flexDirection: "row", alignItems: "center", gap: 3, marginRight: 4 },
  likesText: { fontSize: 11, fontWeight: "700" },
});
